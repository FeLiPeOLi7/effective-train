...
def load_data(filename):
    ...
    with open(filename) as f:#Get the CSV file and Open the CSV file
        csv_reader = csv.reader(f)#Read the CSV file
        next(csv_reader, None)#Skip the header
        #Passing trough each row of the csv file
        for row in csv_reader:
            #Evidence list that we will append, so it will not overwrite our data.
            evidence_row =[
            ...
            #Appending each data point
            evidence.append(evidence_row)#List of lists
            labels.append(1 if row[17] == "TRUE" else 0)#List of integers

    return (evidence, labels)#Return tuple - evidence, labels
...
def train_model(evidence, labels):
    ...
    fitted = neigh.fit(evidence, labels)
