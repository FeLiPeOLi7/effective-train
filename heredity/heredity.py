...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    """
    #For each person update the value of their probabilities by adding "p"
    for person in probabilities:#For each person in the probabilities of that person
        if person in one_gene:#If the person has only one gene
            probabilities[person]["gene"][1] += p
        elif person in two_genes:#If the person has two genes
            probabilities[person]["gene"][2] += p
        else:#The person doesn't have the gene
